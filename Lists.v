...
Theorem snd_fst_is_swap : forall (p : natprod),
  ...
  Proof.
  intros p.
  destruct p as [n m].  (* This exposes the components n and m *)
  simpl.                (* Both sides now simplify to (m, n) *)
  reflexivity.
Qed.
(** [] *)
...
Theorem fst_swap_is_snd : forall (p : natprod),
  ...
  Proof.
  intros p.
  destruct p as [n m].  (* Expose the components n and m of the pair *)
  simpl.                (* fst (swap_pair (n, m)) simplifies to fst (m, n) which is m;
                            snd (n, m) simplifies to m *)
  reflexivity.
Qed.
(** [] *)
...
Fixpoint included (s1 : bag) (s2 : bag) : bool := 
  match s1 with
  | nil => true
  | h :: t => member h s2 && included t (remove_all h s2)
  end.
...
Theorem remove_does_not_increase_count: forall (s : bag),
  ...
  - (* s = n::s' *)
    induction n as [| n' IHn'].
    + (* n = 0 *) simpl. rewrite leb_n_Sn. reflexivity.
...
Theorem update_eq :
  forall (d : partial_map) (x : id) (v: nat),
    ...
    Proof.
    intros d x v.           (* name all three arguments *)
    simpl.                  (* unfolds find x (record x v d) into
                               (if eqb_id x x then Some v else find x d) *)
    rewrite eqb_id_refl.    (* eqb_id x x = true, so now the goal is
                               (if true then Some v else …) = Some v *)
    reflexivity.
  Qed.
(** [] *)
